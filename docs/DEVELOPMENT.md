...
12. Cloud 모듈 캐시 갱신은 planner/runtime.py가 담당한다. 내부 수정 배포에서도 app.BUILD_ID, canvas.CANVAS_VERSION, 컴포넌트 등록 이름을 갱신한다. workspace_build 불일치 시 기존 파일 직렬화/복원으로 호실을 보존하며 최신 모델 객체로 전환한다.
